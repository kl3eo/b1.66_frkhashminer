...
#define KECCAKF_1600_1RND_512(a)                                      \
    do                                                                     \
    {                                                                      \
        uint2 _m0, _m1, _m2, _m3, _m4; \
	_m0.x = a[0].x ^ a[5].x; \
	_m0.y = a[0].y; \
	_m1 = a[1]; \
	_m2 = a[2]; \
	...
	_m4 = a[4]; \
						 \
        const uint2 m0 = _m0 ^ ROTL64_1(_m2, 1); \
        const uint2 m1 = _m1 ^ ROTL64_1(_m3, 1); \
        const uint2 m2 = _m2 ^ ROTL64_1(_m4, 1); \
        const uint2 m3 = _m3 ^ ROTL64_1(_m0, 1); \
        const uint2 m4 = _m4 ^ ROTL64_1(_m1, 1); \
                                                                           \
        const uint2 tmp = a[1] ^ m0;                                       \
                                                                           \
        a[0] ^= m4;                                                        \
        ...
        uint2 m6 = a[1];                                                   \
        a[0] = bitselect(a[0] ^ a[2], a[0], a[1]);                         \
        a[0] ^= as_uint2(Keccak_f1600_RC[0]);                              \
        a[1] = bitselect(a[1] ^ a[3], a[1], a[2]);                     \
        a[2] = bitselect(a[2] ^ a[4], a[2], a[3]);                     \
        a[3] = bitselect(a[3] ^ m5, a[3], a[4]);                       \
        a[4] = bitselect(a[4] ^ m6, a[4], m5);                         \
...
#define KECCAKF_1600_1RND_256(a)                                      \
    do                                                                     \
    {                                                                      \
        uint2 _m0, _m1, _m2, _m3, _m4; \
	_m0 = a[0] ^ a[5] ^ a[10]; \
	_m1 = a[1] ^ a[6] ^ a[11] ^ a[16]; \
	_m2 = a[2] ^ a[7] ^ a[12]; \
	_m3 = a[3] ^ a[8]; \
	_m4 = a[4] ^ a[9]; \
    						 \
